...
if($verified == true){
    if( $topic_header == "products/create" ) {
        if(!empty($product)){
			...
			// Check if we found any existing product
			$existing_data = (isset($comeback['data']) && !empty($comeback['data'])) ? $comeback['data'][0] : [];
			$clsProductId = isset($existing_data['product_id']) ? $existing_data['product_id'] : '';
			$ProductId = $clsProductId;
